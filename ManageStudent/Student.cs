...
namespace Week09Homework {
    ...
    class Student : Member, IFile {
        ...
        public string Record {
            get {
                return $"{Number}|{Name}|{Dept.Code}|{BirthInfo:yyyy,MM,dd}|{AdvisorNumber}|{Year}|{(int)Class}|{(int)RegStatus}|{Address}|{Contact}"; }
            }
        public static Student Restore(string record, Department[] departments) {
            Student student = null;
            try {
                var sdata
                = record.Trim().Split(new char[] { '|' });
                if (sdata.Length == 10) {
                    //0 학번
                    //1 이름
                    //2 학과
                    var dept = departments.FirstOrDefault(
                    m => m != null && m.Code == sdata[2].Trim());
                    student = new Student(sdata[0], sdata[1], dept);
                    //3 생일
                    //참고:DateTime birthinfo = DateTime.ParseExact(
                    // sdata[3], "yyyy,MM,dd", null);
                    ...
                    //4 지도교수
                    student.AdvisorNumber = sdata[4].Trim();
                    //5 학년
